...
//        properties.put("STORE_PATH", "/home/zwy/test");
        ...
        for (int i = 0; i < BUCKET_MESSAGE_COUNT; i++) {
            ...
            messagesForTopic1.add(producer.createBytesMessageToTopic(topic1, (topic1 + i).getBytes())
                    .putHeaders(MessageHeader.MESSAGE_ID, "shv4mdp6z2dkc")
                    .putProperties("PRO_OFFSET", "PRODUCER1_" + i)
                    .putProperties("other", "sdfgghrgfdf"));
            messagesForTopic2.add(producer.createBytesMessageToTopic(topic2, (topic2 + i).getBytes())
                    .putHeaders(MessageHeader.MESSAGE_ID, "shv4mdp6z2dkc")
                    .putProperties("PRO_OFFSET", "PRODUCER2_" + i)
                    .putProperties("other", "df542sdgte5"));
            messagesForQueue1.add(producer.createBytesMessageToQueue(queue1, (queue1 + i).getBytes())
                    .putHeaders(MessageHeader.MESSAGE_ID, "shv4mdp6z2dkc")
                    .putProperties("PRO_OFFSET", "PRODUCER3_" + i)
                    .putProperties("other", "dhghfghdfg"));
            messagesForQueue2.add(producer.createBytesMessageToQueue(queue2, (queue2 + i).getBytes())
                    .putHeaders(MessageHeader.MESSAGE_ID, "shv4mdp6z2dkc")
                    .putProperties("PRO_OFFSET", "PRODUCER4_" + i)
                    .putProperties("other", "ecbtycsdfqe"));
        }
        ...
        System.out.printf("SendNum: %d, SendCost: %dms, SendTps: %d qps\n",
        ...
        //消费样例1，实际测试时会Kill掉发送进程，另取进程进行消费
        {
            ...
            System.out.println(String.format("PullNum: %d, PullCost: %d ms, PullTps: %d qps, TotalTps: %d qps",
                    queue1Offset + topic1Offset,
        ...
        //消费样例2，实际测试时会Kill掉发送进程，另取进程进行消费
        {
            ...
            long T2 = endConsumer - startConsumer;
            System.out.println(String.format("PullNum: %d, PullCost: %d ms, PullTps: %d qps, TotalTps: %d qps",
                    queue2Offset + topic1Offset + topic2Offset,
